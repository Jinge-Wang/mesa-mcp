...
- **Apply the change with `mesa_work_inlist_set(path, name, value)`** — it patches in place
...
- **Verify first.** Confirm the control name, namelist, and default with `mesa_docs_option`
  (or `mesa_docs_search`) against the installed version before writing it.
