...
2. **Locate the right control.** Confirm the exact control name, namelist, and value format with
   `mesa_docs_search` / `mesa_docs_page` for the installed version. Controls live in specific
...
4. **Recompile only if needed.** Inlist-only changes do **not** require `./mk`. Recompile
   (`mesa_env_shell("./mk", "<work dir>")`) only after editing `src/*.f90`.
5. **Decide what to do with old output before a fresh run.** A fresh `./rn` runs over any existing
   `LOGS/`/`photos/`. `mesa_run_start` refuses and lists what's there if output already exists — **ask the
   user**: clean it first with `mesa_work_clear` (confirm-gated), or proceed as-is with
   `mesa_run_start(<work dir>, on_existing="continue")`. **Never clean a multi-phase run between phases** —
...
6. **Run or restart — only with explicit user consent.** Confirm first, then start it **detached**
   (non-blocking) with `mesa_run_start(<work dir>)` for a fresh run, or `mesa_run_start(<work dir>, "./re")` to
...
- `mesa_work_clear` only ever removes run *output* (LOGS/, photos/, png/, run-state files) and
