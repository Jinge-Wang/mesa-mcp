...
5. **Decide what to do with old output before a fresh run.** A fresh `./rn` runs over any existing
   ...
   user**: clean it first with `mesa_clean_workspace` (confirm-gated), or proceed as-is with
...
6. **Run or restart — only with explicit user consent.** Confirm first, then start it **detached**
   ...
   model's history columns); `mesa_stop_run` cancels.
...
- `mesa_clean_workspace` only ever removes run *output* (LOGS/, photos/, png/, run-state files) and
