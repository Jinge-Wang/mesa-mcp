...
Call it through `mesa_execute_shell` with a `path` outside `$MESA_DIR`.
...
| `shmesa grep <string>` | Search the MESA **source** for a string (complements `mesa_search_docs`, which searches the docs). |
