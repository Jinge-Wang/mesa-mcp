...
>
> The worked [antigravity CLI transcript](docs/examples/sample-antigravity-cli.html) demonstrates the agent successfully working out the simulation settings for the first lab of [the MESA Summer School 2026](https://mesastar.org/summer-school-2026-labs/monday/lab-1/) (magnetic braking turned off), without any prior knowledge of the curriculum. While this is a relatively naive problem, it highlights the agent's ability to navigate the MESA documentation, configure the workspace, and debug live errors. The agent does require some further improvement regarding domain-specific nuances (for example, initially navigating the a09 opacity tables before being prompted to use the hand-smoothed nans-removed table for the [AGSS09](https://ui.adsabs.harvard.edu/abs/2009ARA%26A..47..481A/abstract) result). Furthermore, this worked example uses gemini-pro model, which seems to be better at handling errors and interpreting results than flash model, but it takes more rounds and consumes much more tokens than the flash model while both are able to obtain the same correct simulation inlist setups. More case studies with different models are needed.
