...
def fetch_mesa_software(timeout: int = 20) -> dict:
    """Query the MESA Zenodo community for the latest release + per-platform SDKs."""
    try:
        import httpx
    except ImportError:
    ...
    for h in hits:
        ...
        found[kind] = {
            ...
            "title": m.get("title"),
            "html_url": h.get("links", {}).get("self_html"),
            "file": f0.get("key"),
...
def installation_plan(env: dict, timeout: int = 20) -> dict:
    ...
    software = fetch_mesa_software(timeout)
    sdk = None
    if "sdks" in software and plat["sdk_key"]:
        sdk = software["sdks"].get(plat["sdk_key"])
    ...
    return {
        ...
        "latest_release": software.get("release") if "release" in software else None,
        "recommended_sdk": sdk,
        "all_sdks": software.get("sdks") if "sdks" in software else None,
        "software_error": software.get("error"),
